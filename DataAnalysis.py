...
class DataAnalysis:
    ...
    def plot_features(self, plot_types : list, colums = 4 ) -> None:
        """
            Plot features per class label using matplotlib, and it plots the plot_types specified in plot_types.

            :param plot_types: An array specifying the type of plot to generate for each feature.
        ...
        for plot_type in plot_types:
            ...
            rows = number_of_features//colums
            number_of_rows = rows if number_of_features % colums == 0 else rows + 1
            print(number_of_rows , colums)
            fig, axes = plt.subplots(nrows=number_of_rows, ncols=colums, figsize=(rows*5, colums*5))

            print(type(axes),axes)

            for i, feature in enumerate(self.dataset.columns):
                # Produce the subplot for each feature
                ax = axes[i // colums][i % colums]  # One-dimensional array containing references to each subplot
